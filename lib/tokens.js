(function() {
  ...
  module.exports = TokenStore = (function(_super) {
    ...
    TokenStore.prototype.crypt = function(type, email, time) {
      var _crypted;
      if (time == null) {
        time = Date.now();
      }
      _crypted = this.crypto.crypt(this.tknpw, type + ":" + email + ":" + time);
      this.debug("crypted", _crypted);
      return _crypted;
    };

    TokenStore.prototype.decrypt = function(token) {
      var email, ret, time, type, _ref, _str;
      _str = this.crypto.decrypt(this.tknpw, token);
      this.debug("decrypted", _str);
      _ref = _str.split(":"), type = _ref[0], email = _ref[1], time = _ref[2];
      ret = {
        type: type,
        email: email,
    ...
    TokenStore.prototype._create = function(type, mail, cb) {
      var rM, _key, _token,
        _this = this;
      _token = this.crypt(type, mail);
      _key = this._tRedisKey(mail);
      rM = [];
      rM.push(["LPUSH", _key, _token]);
      rM.push(["LTRIM", _key, 0, this.config.maxUserTokens - 2]);
